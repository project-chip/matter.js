 ...
 * position relative to the device (e.g. the temperature sensor in the top drawer of a refrigerator, or location of the
 * buttons on a multi-button switch device). Note the difference with Chapter 5, Common Direction Semantic Tag
 * Namespace.
 *
 * When multiple endpoints are used for device types, and the associated consumer-facing locations of those endpoints
 * are organized in a straight line, grid or matrix, these endpoints SHOULD be allocated in top-to-bottom,
 * left-to-right order.
 *
