 ...
 */
export class RefrigeratorAndTemperatureControlledCabinetModeServer extends RefrigeratorAndTemperatureControlledCabinetModeBehavior {
    ...
    #assertSupportedModes() {
        ...
        if (
            !this.state.supportedModes.some(({ modeTags }) =>
                modeTags.some(({ value }) => value === RefrigeratorAndTemperatureControlledCabinetMode.ModeTag.Auto),
            )
        ) {
            throw new Error("Provided supportedModes need to include at least Auto mode tag");
        }
    }
