...
export class CryptoJs extends Crypto {
    ...
    pbkdf2(secret: ByteArray, salt: ByteArray, iteration: number, keyLength: number): Promise<ByteArray> {
        return new Promise<ByteArray>((resolver, rejecter) => {
            crypto.pbkdf2(secret, salt, iteration, keyLength, CRYPTO_HASH_ALGORITHM, (error: Error, key: ByteArray) => {
    ...
    hkdf(
        ...
        return new Promise<ByteArray>((resolver, rejecter) => {
            crypto.hkdf(CRYPTO_HASH_ALGORITHM, secret, salt, info, length, (error: Error, key: ByteArray) => {
                if (error !== null) rejecter(error);
                resolver(new ByteArray(key));
            });
        });
