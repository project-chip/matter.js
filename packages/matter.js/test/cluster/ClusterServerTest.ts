...
describe("ClusterServer structure", () => {
    describe("correct attribute servers are used and exposed", () => {
        ...
        it("Optional existing fixed attribute, hast also get and no set, but potentially undefined", async () => {
            ...
            expect(basic.attributes.manufacturingDate instanceof FixedAttributeServer && basic.attributes.manufacturingDate.isFixed).toBeTruthy();
            // guard needed because as per types optional are potentially undefined
            ...
            expect(basic2?.attributes.manufacturingDate instanceof FixedAttributeServer && basic2.attributes.manufacturingDate.isFixed).toBeTruthy();
            // guard needed because as per types optional are potentially undefined
        ...
        it("Optional non-existing attribute returns undefined but do not exist in named object", async () => {
            ...
            expect(() => basic.setLocalConfigDisabledAttribute(true)).toThrow(
                new ImplementationError("Attribute localConfigDisabled is optional and not initialized. To use it please initialize it first.")
            );
            expect(() => basic.subscribeLocalConfigDisabledAttribute(() => { /* ignore */ })).toThrow(
                new ImplementationError("Attribute localConfigDisabled is optional and not initialized. To use it please initialize it first.")
            );
            ...
            expect(() => basic2?.subscribeLocalConfigDisabledAttribute(() => { /* ignore */ })).toThrow(
                new ImplementationError("Attribute localConfigDisabled is optional and not initialized. To use it please initialize it first.")
            );
        });
