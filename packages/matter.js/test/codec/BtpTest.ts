...
describe("BtpCodec", () => {
    ...
    describe("encode", () => {
        ...
        it("encodes a valid btp packet where both ackNumber and messageLength are undefined", () => {
            ...
            expect(result).toEqual(ByteArray.fromHex("06000400000049b6a902a9a5773dbb8cafd90120a7c7000015300120cb0c120a3499327ddaec4ebe60889df0f1bf80d8a4dea1dd6ffef16ef58ecafe25028e17240300280418"))
        });

        it("encodes a valid btp packet where ackNumber is undefined", () => {
            const result = BtpCodec.encodeBtpPacket(DECODED_PACKET_1);

            expect(result).toEqual(ByteArray.fromHex("050044000400000049b6a902a9a5773dbb8cafd90120a7c7000015300120cb0c120a3499327ddaec4ebe60889df0f1bf80d8a4dea1dd6ffef16ef58ecafe25028e17240300280418"))
        });

        it("encodes a valid btp packet where messageLength is undefined", () => {
            const result = BtpCodec.encodeBtpPacket(DECODED_PACKET_3);

            expect(result).toEqual(ByteArray.fromHex("0e00000400000049b6a902a9a5773dbb8cafd90120a7c7000015300120cb0c120a3499327ddaec4ebe60889df0f1bf80d8a4dea1dd6ffef16ef58ecafe25028e17240300280418"))
        });
    });
    ...
    describe("Errors", () => {
        it("incorrect headers in handshake request", () => {
            expect(() => BtpCodec.decodeBtpHandshakeRequest(ByteArray.fromHex("0d6c04000000b90006")))
                .toThrow(new BtpProtocolError("Header for expected BTP Handshake Request is incorrect."));
        });
        ...
        it("incorrect management opcode in handshake request", () => {
            expect(() => BtpCodec.decodeBtpHandshakeRequest(ByteArray.fromHex("656d04000000b90006")))
                .toThrow(new BtpProtocolError("Management Opcode for BTP Handshake Request is incorrect."));
        });
        ...
        it("no valid version provided in request handshake", () => {
            expect(() => BtpCodec.decodeBtpHandshakeRequest(ByteArray.fromHex("656c00000000b90006")))
                .toThrow(new BtpProtocolError("No valid version provided."));
        });
        ...
        it("ack number shouldn't be present if hasAckNumber is false", () => {
            expect(() => BtpCodec.encodeBtpPacket(DECODED_PACKET_4))
                .toThrow(new BtpProtocolError("Ack number shouldn't be set because header flag is not set."));
        });

        it("ack number should be present if hasAckNumber is true", () => {
            expect(() => BtpCodec.encodeBtpPacket(DECODED_PACKET_5))
                .toThrow(new BtpProtocolError("Ack number needs to be set because header flag is set."));
        });

        it("message length shouldn't be present if beginning segment is false", () => {
            expect(() => BtpCodec.encodeBtpPacket(DECODED_PACKET_6))
                .toThrow(new BtpProtocolError("Message length shouldn't be set because the package is not a beginning segment."));
        });

        it("message length should be present if beginning segment is true", () => {
            expect(() => BtpCodec.encodeBtpPacket(DECODED_PACKET_7))
                .toThrow(new BtpProtocolError("Message length needs to be set because packet is a beginning segment."));
        });

        it("packet should not have management opcode", () => {
            expect(() => BtpCodec.encodeBtpPacket(DECODED_PACKET_8))
                .toThrow(new BtpProtocolError("Please use the specific methods to encode a Handshake packet"));
        });
    });
});
