...
function Features(definition: { [code: string]: string }): AttributeModel {
    return new AttributeModel({
        ...
        children: Object.entries(definition).map(([name, description]) => new DatatypeModel({
            name,
            description
        }))
    });
...
const AllTests = Tests({
    conformance: Tests({
        ...
        choice: Tests({
            ...
            "exactly two": Tests(Fields({ name: "Test1", conformance: "O.a2" }, { name: "Test2", conformance: "O.a2" }, { name: "Test3", conformance: "O.a2" }), {
                "allows two fields": {
            ...
            "one or more": Tests(Fields({ name: "Test1", conformance: "O.a+" }, { name: "Test2", conformance: "O.a+" }), {
                "allows one field": {
                    record: { test1: 1234 }
                },
