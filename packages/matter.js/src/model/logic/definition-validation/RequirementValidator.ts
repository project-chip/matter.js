...
ModelValidator.validators[RequirementElement.Tag] =
    class RequirementValidator extends ModelValidator<RequirementModel> {
        override validate() {
                ...
                switch (this.model.element) {
                    ...
                    case RequirementElement.ElementType.ServerCluster:
                        if (parentTag !== ElementTag.DeviceType) {
                            this.error("ILLEGAL_REQUIREMENT_PARENT", `Requirement type ${this.model.type} must be parented by device type`);
                        }
                        break;
                    ...
                    case RequirementElement.ElementType.Event:
                        if (parentTag !== ElementTag.Requirement) {
                            this.error("ILLEGAL_REQUIREMENT_PARENT", `Requirement type ${this.model.type} must be parented by cluster requirement`);
                        }
                        break;
                }
            }
