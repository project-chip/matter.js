 ...
 */
export const ModeSelectBase = BaseClusterComponent({
    id: 0x50,
    name: "ModeSelect",
    revision: 1,
    ...
    attributes: {
         ...
         * Select cluster for the amount of sugar to add. In this case, the first instance can have the description
         * Milk and the second instance can have the description Sugar. This allows the user to tell the purpose of
 ...
 *
 * ModeSelectCluster supports optional features that you can enable with the ModeSelectCluster.with() factory method.
 *
 * @see {@link MatterApplicationClusterSpecificationV1_1} § 1.8
 */
export const ModeSelectCluster = ExtensibleCluster({
    ...ModeSelectBase,

    /**
     * Use this factory method to create a ModeSelect cluster with support for optional features. Include each
     * {@link ModeSelectFeature} you wish to support.
     *
     * @param features the optional features to support
     * @returns a ModeSelect cluster with specified features enabled
     * @throws {IllegalClusterError} if the feature combination is disallowed by the Matter specification
     */
    factory: <T extends `${ModeSelectFeature}`[]>(...features: [...T]) => {
        validateFeatureSelection(features, ModeSelectFeature);
        const cluster = Cluster({ ...ModeSelectBase, supportedFeatures: BitFlags(ModeSelectBase.features, ...features) });
        return cluster as unknown as ModeSelectExtension<BitFlags<typeof ModeSelectBase.features, T>>;
    }
});

export type ModeSelectExtension<SF extends TypeFromPartialBitSchema<typeof ModeSelectBase.features>> =
