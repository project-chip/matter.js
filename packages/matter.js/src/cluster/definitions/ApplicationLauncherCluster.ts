 ...
 */
export enum ApplicationLauncherFeature {
    /**
     * ApplicationPlatform
     *
 ...
 */
export const ApplicationLauncherBase = BaseClusterComponent({
    id: 0x50c,
 ...
 */
export const ApplicationLauncherCluster = ExtensibleCluster({
    ...ApplicationLauncherBase,

    /**
     * Use this factory method to create an ApplicationLauncher cluster with support for optional features. Include
     * each {@link ApplicationLauncherFeature} you wish to support.
     *
     * @param features the optional features to support
     * @returns an ApplicationLauncher cluster with specified features enabled
     * @throws {IllegalClusterError} if the feature combination is disallowed by the Matter specification
     */
    factory: <T extends `${ApplicationLauncherFeature}`[]>(...features: [...T]) => {
        validateFeatureSelection(features, ApplicationLauncherFeature);
        const cluster = Cluster({
            ...ApplicationLauncherBase,
            supportedFeatures: BitFlags(ApplicationLauncherBase.features, ...features)
        });
        return cluster as unknown as ApplicationLauncherExtension<BitFlags<typeof ApplicationLauncherBase.features, T>>;
    }
});

export type ApplicationLauncherExtension<SF extends TypeFromPartialBitSchema<typeof ApplicationLauncherBase.features>> =
