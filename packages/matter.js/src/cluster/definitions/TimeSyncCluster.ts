 ...
 */
export const TimeSyncBase = BaseClusterComponent({
    id: 0x38,
    name: "TimeSync",
    revision: 1,
...
export const NtpClientComponent = ClusterComponent({
    attributes: {
        /**
         * The default NTP server the server’s Node may use if other time sources are unavailable. This attribute may
...
export const TimeZoneComponent = ClusterComponent({
    attributes: {
         ...
         * offset configurations to allow Nodes to handle scheduled regulatory time zone changes. This attribute shall
         * NOT be used to indicate daylight savings time changes (see Section 11.16.8.7, “DSTOffset Attribute” for
         ...
         *     start or end dates.
         *
         * Upon writing this attribute, the server shall recompute its LocalTime, taking into account the Offset of the
         * currently used TimeZoneStruct.
         *
    ...
    events: {
        /**
         * This event shall be generated when the server stops applying the current DSTOffset and there are no entries
         * in the list with a larger ValidStarting time, indicating the need to possibly get new DST data.
         *
         ...
         *
         * This event contains a boolean predicate that indicates whether the server is applying the DST offset. When
         * the value is "true", the current DST offset is being applied (i.e, daylight savings time is applied, as
...
export const NtpServerComponent = ClusterComponent({
    attributes: {
        /**
         * If the server is running an NTP server, this value shall be the port number for the service. If the server
         * is not currently running an NTP server, this value shall be null.
         *
         * This attribute shall be present if this server is capable of providing an NTP server instance. See Section
         * 11.16.15, “Acting as an NTP Server” for more information.
         *
 ...
 *
 * This section describes a mechanism for Nodes to achieve and maintain time synchronization. The Time Cluster provides
 * attributes for reading a Node’s current time. It also allows Administrators to
 *
 ...
 */
export const TimeSyncCluster = ExtensibleCluster({
    ...TimeSyncBase,

    /**
     * Use this factory method to create a TimeSync cluster with support for optional features. Include each
     * {@link TimeSyncFeature} you wish to support.
     *
     * @param features the optional features to support
     * @returns a TimeSync cluster with specified features enabled
     * @throws {IllegalClusterError} if the feature combination is disallowed by the Matter specification
     */
    factory: <T extends `${TimeSyncFeature}`[]>(...features: [...T]) => {
        validateFeatureSelection(features, TimeSyncFeature);
        const cluster = Cluster({ ...TimeSyncBase, supportedFeatures: BitFlags(TimeSyncBase.features, ...features) });
        extendCluster(cluster, TimeZoneComponent, { timeZone: true });
        extendCluster(cluster, NtpServerComponent, { ntpServer: true });
        return cluster as unknown as TimeSyncExtension<BitFlags<typeof TimeSyncBase.features, T>>;
    }
});

export type TimeSyncExtension<SF extends TypeFromPartialBitSchema<typeof TimeSyncBase.features>> =
    ClusterForBaseCluster<typeof TimeSyncBase, SF>
