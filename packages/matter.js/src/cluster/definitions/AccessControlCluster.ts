...
export const TlvAccessControlEntryStruct = TlvObject({
     ...
     * Administer is the highest privilege, and is special as it pertains to the administration of privileges itself,
     * via the Access Control Cluster.
     *
     ...
     * successfully authenticates via AuthMode. The subjects list shall NOT be empty if the entry’s AuthMode is PASE.
     *
     ...
     *
     * For CASE authentication, the Subject ID is a distinguished name within the Operational Certificate shared during
     * CASE session establishment, the type of which is determined by its range to be one of:
     *
     ...
     * error and the entry shall NOT be created.
     *
     * A single target shall contain at least one field (Cluster, Endpoint, or DeviceType), and shall NOT contain both
     * an Endpoint field and a DeviceType field.
     *
     * A target grants access based on the presence of fields as follows:
     *
     * Target Semantics
     *
     * An empty targets list indicates a wildcard: that is, this entry shall grant access to all cluster instances on
     * all endpoints on this Node.
     *
...
export const TlvAccessControlEntryChangedEvent = TlvObject({
     ...
     *
     * Exactly one of AdminNodeID and AdminPasscodeID shall be set, depending on whether the change occurred via a CASE
     * or PASE session; the other shall be null.
     *
 ...
 * used to manage and enforce Access Control for the Node’s endpoints and their associated cluster instances. Access to
 * this Access Control Cluster itself requires a special Administer privilege level, such that only Nodes granted such
 * privilege (hereafter termed "Administrators") can manage the Access Control Cluster.
 *
...
export const AccessControlCluster = Cluster({
    ...
    attributes: {
         ...
         * given implementation, it is recommended to only use the minimum value required and avoid reporting a higher
         * value than the required minimum.
         *
