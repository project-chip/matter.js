 ...
 *
 * All value in the PIN/RFID code shall be ASCII encoded regardless if the PIN/RFID codes are number or characters. For
 * example, code of “1, 2, 3, 4” shall be represented as 0x31, 0x32, 0x33, 0x34.
 *
...
export const DoorLockBase = BaseClusterComponent({
    ...
    attributes: {
        /**
         * The LockState Attribute may be NULL if the lock hardware does not currently know the status of the locking
         ...
         *   • LockState
         *
         * When the LockState attribute is part of a Scene table, the attribute is treated as a writable command; that
         * is, setting the LockState to lock will command the lock to lock, and setting the LockState to unlock will
         * command the lock to unlock. Setting the LockState attribute to “not fully locked” is not supported. The
         * Transition Time field in the Scene table will be treated as a delay before setting the LockState attribute;
         ...
         * modes NOT supported by a lock shall be set to one. The value of the OperatingMode enumeration defines the
         * related bit to be set, as
         *
         * @see {@link MatterApplicationClusterSpecificationV1_1} § 5.2.3.24
         */
         ...
         * features. If it is set to FALSE then local programming is disabled on the door lock for those features whose
         * bit is set to 0 in the LocalProgrammingFeatures attribute. Local programming shall be enabled by default.
         *
         ...
         * If a door lock doesn’t support disabling one aspect of local programming it shall return CONSTRAINT_ERROR
         * during a write operation of this attribute. If the EnableLocalProgramming attribute is set to True then all
         * local programming features shall be enabled regardless of the bits set to 0 in this attribute.
         *
         ...
         * turn on/off alarms for particular functions. Alarms for an alarm group are enabled if the associated alarm
         * mask bit is set. Each bit represents a group of alarms. Entire alarm groups can be turned on or off by
    ...
    events: {
         ...
         * lock. The alarms available for the door lock cluster are listed in the AlarmCodeEnum section below.
         *
         * @see {@link MatterApplicationClusterSpecificationV1_1} § 5.2.5.1
         */
...
export const LoggingComponent = ClusterComponent({
    attributes: {
         ...
         * retrieval. Logging events can be but not limited to Tamper Alarm, Lock, Unlock, AutoRelock, User Code Added,
         * User Code Cleared, Schedule Added, and Schedule Cleared. For a full detail of all the possible alarms and
...
export const UserComponent = ClusterComponent({
    attributes: {
         ...
         * shall remain valid after its first use before expiring. When the credential expires the UserStatus for the
         * corresponding user record shall be set to OccupiedDisabled.
         *
...
export const PinCredentialComponent = ClusterComponent({
    attributes: {
         ...
         * determines the behavior of the server’s TX operation. If it is false, then it is not ok for the device to
         * send PIN in any messages over the air.
         *
...
export const RfidCredentialComponent = ClusterComponent({
    attributes: {
         ...
         * RFID code range specified by the manufacturer, if media anti-collision identifiers (UID) are used as RFID
...
export const PinCredentialOrRfidCredentialComponent = ClusterComponent({
    attributes: {
        /**
         * The number of incorrect Pin codes or RFID presentment attempts a user is allowed to enter before the lock
         * will enter a lockout state. The value of this attribute is compared to all failing forms of credential
         * presentation, including Pin codes used in an Unlock Command when RequirePINforRemoteOperation is set to
         * true. Valid range is 1-255 incorrect attempts. The lockout state will be for the duration of
         ...
         * Device can shut down to lock user out for specified amount of time. (Makes it difficult to try and guess a
         * PIN for the device.) If the attribute accepts writes and an attempt to write the attribute to 0 is made, the
...
export const NotificationAndPinCredentialComponent = ClusterComponent({
    attributes: {
        /**
         * Event mask used to turn on and off the transmission of keypad operation events. This mask DOES NOT apply to
         * the storing of events in the event log. This mask only applies to the Operation Event Notification Command.
         *
         * This mask DOES NOT apply to the Events mechanism of this cluster.
         *
         * @see {@link MatterApplicationClusterSpecificationV1_1} § 5.2.3.40
...
export const NotificationComponent = ClusterComponent({
    attributes: {
        /**
         * Event mask used to turn on and off the transmission of remote operation events. This mask DOES NOT apply to
         * the storing of events in the event log. This mask only applies to the Operation Event
         *
...
export const NotificationAndRfidCredentialComponent = ClusterComponent({
    attributes: {
        /**
         * Event mask used to turn on and off RFID operation events. This mask DOES NOT apply to the storing of events
         * in the event log. This mask only applies to the Operation Event Notification Command.
         *
         * This mask DOES NOT apply to the Events mechanism of this cluster.
         *
         * @see {@link MatterApplicationClusterSpecificationV1_1} § 5.2.3.43
...
/**
 * A DoorLockCluster supports these elements if it supports feature RfidCredential and it doesn't support feature USR.
 */
 ...
 */
export const DoorLockCluster = ExtensibleCluster({
    ...DoorLockBase,

    /**
     * Use this factory method to create a DoorLock cluster with support for optional features. Include each
     * {@link DoorLockFeature} you wish to support.
     *
     * @param features the optional features to support
     * @returns a DoorLock cluster with specified features enabled
     * @throws {IllegalClusterError} if the feature combination is disallowed by the Matter specification
     */
    factory: <T extends `${DoorLockFeature}`[]>(...features: [...T]) => {
        validateFeatureSelection(features, DoorLockFeature);
        const cluster = Cluster({ ...DoorLockBase, supportedFeatures: BitFlags(DoorLockBase.features, ...features) });
        ...
        extendCluster(cluster, PinCredentialOrRfidCredentialComponent, { pinCredential: true }, { rfidCredential: true });
...
export type DoorLockExtension<SF extends TypeFromPartialBitSchema<typeof DoorLockBase.features>> =
    ClusterForBaseCluster<typeof DoorLockBase, SF>
