...
export const ScenesClusterHandler: () => ClusterServerHandlers<typeof ScenesCluster> = () => {
    ...
    return {
        addScene: async ({ request: { groupId, sceneId, transitionTime, sceneName, extensionFieldSets }, attributes: { sceneCount }, session, message: { packetHeader: { sessionType } }, endpoint }) => {
            ...
            const result = addSceneLogic(endpoint.getId(), groupId, sceneId, transitionTime, sceneName, extensionFieldSets, 0, (session as SecureSession<MatterDevice>).getAssociatedFabric());
        ...
        enhancedAddScene: async ({ request: { groupId, sceneId, transitionTime, sceneName, extensionFieldSets }, attributes: { sceneCount }, session, message: { packetHeader: { sessionType } }, endpoint }) => {
            ...
            const result = addSceneLogic(endpoint.getId(), groupId, sceneId, Math.floor(transitionTime / 10), sceneName, extensionFieldSets, transitionTime % 10, (session as SecureSession<MatterDevice>).getAssociatedFabric());
            if (result.status === StatusCode.Success) {
                sceneCount.updated(session as SecureSession<MatterDevice>);
            }
            return result;
