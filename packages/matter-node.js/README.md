...
To make sure that matter.js is compliant to the Matter specifications we implemented automated testing options by using the test suites included in chip-tool. These tests are not part of the standard test run describe above because they need prerequisits and also a lot of ressources.

Please refer to the  [test-chip](./test-chip/README.md) folder for prerequisits and details.
