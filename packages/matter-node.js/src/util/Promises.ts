...
export async function getPromiseResolver<T>(): Promise<{ promise: Promise<T>, resolver: (value: T) => void, rejecter: (reason?: any) => void }> {
    let promise: Promise<T>;
    return new Promise<{ resolver: (value: T) => void, rejecter: (reason?: any) => void }>(
        resolve => {
