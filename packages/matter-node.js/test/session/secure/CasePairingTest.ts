...
describe("CasePairing", () => {
    describe("pair", () => {
        it("generates the right bytes for sigma 2", async () => {
            const sigma1Bytes = ByteArray.fromHex(
            ...
            const ipk = ByteArray.fromHex("0c677d9b5ac585827b577470bd9bd516");
            const random = ByteArray.fromHex("75d1443943a23371699bb017958a87e9ec6bbcad5f990aa3822a45bec7786489");
            const sharedSecret = ByteArray.fromHex("08ec3182f7111854fd22fe527951527e4fe905d29a959f99013279cd9c9299f1");
            const pubKey = ByteArray.fromHex(
            ...
            const peerPubKey = ByteArray.fromHex(
            ...
            const fabricPubKey = ByteArray.fromHex(
            ...
            const sigma2Key = await cryptoNode.hkdf(sharedSecret, sigma2Salt, ByteArray.fromString("Sigma2"));

            assert.equal(sigma2Key.toHex(), "7ed5e720195c511dc2d97535e262f935");
            ...
            const signature = ByteArray.fromHex(
            ...
            const encryptedDataPlain = TlvEncryptedDataSigma2.encode({
                nodeOpCert: noc,
                signature,
                resumptionId: ByteArray.fromHex("8731f8cec507136df7558fca9360e9fc"),
            });

            assert.equal(
                encryptedDataPlain.toHex(),
            ...
            const encrypted = cryptoNode.encrypt(sigma2Key, encryptedDataPlain, ByteArray.fromString("NCASE_Sigma2N"));
        ...
        it("generates the right signature", () => {
            ...
            const signatureData = ByteArray.fromHex(
        ...
        it("generates the right bytes for sigma 3", async () => {
            const ecdhPublicKey = ByteArray.fromHex(
            ...
            const peerEcdhPublicKey = ByteArray.fromHex(
            ...
            const sigma2Bytes = ByteArray.fromHex(
            ...
            const sigma3Bytes = ByteArray.fromHex(
            ...
            const identityProtectionKey = ByteArray.fromHex("0c677d9b5ac585827b577470bd9bd516");
            const sharedSecret = ByteArray.fromHex("c1466cab85d7809732786f63cb3b014a51cd49d01f494fcd56665ba8e00e5502");
            const peerEncrypted = ByteArray.fromHex(
            ...
            const sigma3Key = await cryptoNode.hkdf(sharedSecret, sigma3Salt, ByteArray.fromString("Sigma3"));

            assert.equal(sigma3Key.toHex(), "5ba760e89d08efcfe22dbe5832684415");

            const peerEncryptedData = cryptoNode.decrypt(
                sigma3Key,
                peerEncrypted,
                ByteArray.fromString("NCASE_Sigma3N"),
            );

            assert.equal(
                peerEncryptedData.toHex(),
            ...
            const decryptKey = await cryptoNode.hkdf(
                sharedSecret,
                secureSessionSalt,
                ByteArray.fromString("SessionKeys"),
                16,
            );
