...
describe("MdnsServer", () => {
    ...
    let send: (message: BufferSource, remoteIp: string, netInterface: string) => void;
    let onResponse: (message: BufferSource, netInterface?: string, unicastTarget?: string) => Promise<void>;
    const udpServerSimulator = {
        onMessage(listener: (message: BufferSource, remoteIp: string, netInterface: string) => void) {
            send = listener;
        },

        send(message: BufferSource, netInterface?: string, unicastTarget?: string) {
    ...
    describe("Server responds to standard queries for different types", () => {
        it("server responds to an ANY query", async () => {
            ...
            onResponse = async (message: BufferSource, netInterface?: string, uniCastTarget?: string) => {
