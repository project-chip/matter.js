...
export class KeySets<T extends OperationalKeySet> extends BasicSet<T> {
    ...
    readonly #sessions = new Map<number, { keySetId: number; key: BufferSource }[]>();
    ...
    allKeysForId(keySetId: number) {
        ...
        const operationalKeys = Array<{ key: BufferSource; sessionId?: number; startTime: number | bigint }>();
        const {
    ...
    async sessionIdFromKey(crypto: Crypto, operationalGroupKey: BufferSource) {
...
export type OperationalKeySet = GroupKeySet & {
    operationalEpochKey0: BufferSource;
    groupSessionId0: number | null;
    operationalEpochKey1: BufferSource | null;
    groupSessionId1: number | null;
    operationalEpochKey2: BufferSource | null;
